...
__global__ void RoiPoolingGpuKernel(   const float* data_array, 
    ...
    for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < N;
        ...
        float kernel_width = static_cast<float>(roi_width)
                             / static_cast<float>(pooling_width);
        float kernel_height = static_cast<float>(roi_height)
                             / static_cast<float>(pooling_height);
        
        // Iterate through each pooling region
        for (int rx = 0; rx < pooling_width; ++rx) {
            for (int ry = 0; ry < pooling_height; ++ry) {
                int hstart = static_cast<int>(floor(static_cast<float>(ry)
                                        * kernel_height));
                int wstart = static_cast<int>(floor(static_cast<float>(rx)
                                        * kernel_width));
                int hend = static_cast<int>(ceil(static_cast<float>(ry + 1)
                                     * kernel_height));
                int wend = static_cast<int>(ceil(static_cast<float>(rx + 1)
                                     * kernel_width));
